...
public:
  ...
  boost::system::error_code open(const url& u, boost::system::error_code& ec)
  {
    ...
    for (auto&& p: request_headers)
      request_stream << p.first << ": " << p.second << "\r\n";
    ...
    // 200 <= successful < 300 
    if (status_code < http::errc::ok || status_code >= http::errc::multiple_choices)
  ...
  public:
        ...
        std::size_t bytes_transferred = 0)
    {
      ...
      if (!parse_http_headers(headers_.begin(), headers_.end(),
            content_type_, content_length_, location_)
            && options_.get_option<urdl::http::request_method>().value() != "HEAD")
      {
        ec = http::errc::malformed_response_headers;
        handler_(ec);
        return;
      ...
      // Check the response code to see if we got the page correctly.
      // 200 <= successful < 300 
      if (status_code_ < http::errc::ok || status_code_ >= http::errc::multiple_choices)
