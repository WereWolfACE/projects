...
class TheoraPlayerExport TheoraVideoClip
{
	...
	TheoraFrameQueue* mFrameQueue = nullptr;
	TheoraAudioInterface* mAudioInterface = nullptr;
	TheoraDataSource* mStream = nullptr;

	TheoraTimer *mTimer = nullptr, *mDefaultTimer = nullptr;

	TheoraWorkerThread* mAssignedWorkerThread = nullptr;

	// benchmark vars
	int mNumDroppedFrames = 0, mNumDisplayedFrames = 0;

	int mTheoraStreams = 0, mVorbisStreams = 0;	// Keeps track of Theora and Vorbis Streams

	int mNumPrecachedFrames = 0;

	int mSeekFrame = -1; //! stores desired seek position as a frame number. next worker thread will do the seeking and reset this var to -1
	float mDuration = 0, mFrameDuration = 0, mFPS = 0;
	std::string mName;
	...
	bool mAutoRestart = false;
	bool mEndOfFile = false, mRestarted = false;
	int mIteration = 0, mLastIteration = 0; //! used to detect when the video restarted

	float mUserPriority = 0; //! TODO implementation

	TheoraInfoStruct* mInfo = nullptr; // a pointer is used to avoid having to include theora & vorbis headers

	TheoraMutex* mAudioMutex = nullptr; //! syncs audio decoding and extraction
...
public:
	...
	//! benchmark function
	int getNumDisplayedFrames() const { return mNumDisplayedFrames; }
	//! benchmark function
	int getNumDroppedFrames() const { return mNumDroppedFrames; }
	...
	*/
	...
	int getNumPrecachedFrames() const;
	//! returns the number of ready frames in the frame queue
	int getNumReadyFrames() const;
	...
	bool getAutoRestart() const { return mAutoRestart; }
	...
	TheoraOutputMode getOutputMode() const;
	/**
